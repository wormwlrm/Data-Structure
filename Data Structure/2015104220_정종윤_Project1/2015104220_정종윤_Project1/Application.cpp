...
void Application::load_customer_file()
{
	...
	while (loadCustomer >> _Id >> _Password >> _Name)  // 파일 입력이 없을 때 까지 데이터를 불러옴
	{
		Customer* temp = new Customer(); // 임시 고객 변수 동적 할당
		temp->setCustomer(_Id, _Password, _Name); // 받은 데이터 설정
				...
				for (int j = 0; j < 3; j++) // 중분류 카테고리 설정
				{
					if (Larges[i].Middles[j].MiddleCategory == temp->getCategoryMiddle()) // 일치할 경우
					{
						for (int k = 0; k < 3; k++) // 소분류 카테고리 설정
						{
							...
							{
								Larges[i].Middles[j].Smalls[k].productList.Add(temp);		 // 물건 배열에 추가						
...
void Application::Add_Customer()
{
	...
	cout << "비밀번호 : "; cin >> _PW;
	cout << "이름     : "; cin >> _N;
	Temp->setCustomer(_Id, _PW, _N); // 정보 입력
...
void Application::printCustomerInfo()
{
	...
	cout << "이름     :" << setw(10) << LoginedCustomer->getName(); //로그인된 유저의 이름 출력
	cout << "\n─────────────" << endl;
}
