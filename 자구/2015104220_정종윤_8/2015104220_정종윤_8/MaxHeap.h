...
{
	if (isFull()) // 힙이 가득 차있을 경우 예외처리
		return 0;

	m_pHeap[m_iLastNode++] = item; // 마지막에 삽입
	ReheapUp(0, m_iLastNode - 1); // 아래에서부터 힙정렬
	return 1;
}
...
void MaxHeap<T>::ReheapDown(int iparent, int ibottom)
{
	...
	if (left <= ibottom)
	{
		...
		if (m_pHeap[iparent] < m_pHeap[max]) //부모의 값이 더 작을 경우
		{
			swap(m_pHeap[iparent], m_pHeap[max]);	//위치 바꿈
			ReheapDown(max, ibottom);	//재귀 반복 
		}
	}
}
...
void MaxHeap<T>::ReheapUp(int iroot, int ibottom)
{
	...
	{
		parent = (ibottom - 1) / 2;
		if (m_pHeap[parent] < m_pHeap[ibottom]) // 부모의 값이 더 작을 경우
		{
...
void MaxHeap<T>::Delete(T item, bool& found, int iparent)
{
	if (m_pHeap[iparent] == item)	//값이 일치할 경우
	{
		...
		ReheapDown(iparent, m_iLastNode - 1);  // 위에서부터 힙정렬
		found = true;
		return;
	}
	else
