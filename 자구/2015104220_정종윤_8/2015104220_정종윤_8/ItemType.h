...
public:
	...
	*/
	...
	/**
	*	@brief	Check if two item is same by comparing ID.
	*	@pre	Two item was initialized.
	*	@param	data	Target Item
	*	@return	Return trueifs two item is same, otherwise 0.
	*/
	bool operator==(const ItemType data);

	/**
	*	@brief	Check if this* is big and same with target by comparing ID.
	*	@pre	Two item was initialized.
	*	@param	data	Target Item
	*	@return	Return true if two item is big or same, otherwise 0.
	*/
	bool operator>=(const ItemType data);

	/**
	*	@brief	Check if this* is small and same with target by comparing ID.
	*	@pre	Two item was initialized.
	*	@param	data	Target Item
	*	@return	Return true if two item is small, otherwise 0.
	*/
	bool operator<=(const ItemType data);
	
	/**
	*	@brief	Check if this* is bigger than target by comparing ID.
	*	@pre	Two item was initialized.
	*	@param	data	Target Item
	*	@return	Return true if this* is bigger than target, otherwise 0.
	*/
	bool operator>(const ItemType data);

	/**
	*	@brief	Check if this* is smaller than target by comparing ID.
	*	@pre	Two item was initialized.
	*	@param	data	Target Item
	*	@return	Return true if this* is smaller than target, otherwise 0.
	*/
	bool operator<(const ItemType data);

	/**
	*	@brief	Check if this* is not equal with target by comparing ID.
	*	@pre	Two item was initialized.
	*	@param	data	Target Item
	*	@return	Return true if this* is not equal with target, otherwise 0.
	*/
	bool operator!=(const ItemType data);
	
	/**
	*	@brief	Print Information of Item
	*	@pre	Information must be initialized
	*	@post	Print Information
	*	@return	Return ofstream
	*/
	friend ostream &operator<<(ostream& out, ItemType data)
	{
		out << "\tID   : " << data.GetId() << endl;
		out << "\tName   : " << data.GetName() << endl;
